...
@section('content')
				...
				<div class="blog-post">
		            <h2 class="blog-post-title">
		            	{{ __('messages.tag') }} : 
		            	<a href="{{ route('lessonsTag.show', $tag->slug) }}">
		            		{{ $tag->name }}
		            	</a>
		            </h2>
...
@section('sidebar')
				...
				<div class="tags-list">
			    	@foreach($tags as $tag)
			    		<li><a href="{{ route('lessonsTag.show', $tag->slug) }}">{{ $tag->translate(app()->getLocale())->name }} <button type="button" class="btn btn-warning btn-round-xs btn-xs"> {{ $tag->lessons_count }}</button></a></li>
					@endforeach
				</div>
